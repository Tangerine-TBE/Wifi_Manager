...
import com.example.module_base.utils.Rx.RxFileTool.Companion.rootPath
...
//        String needDelFiel = FILE_SUFFIX.format(getDateBefore());
    ...
    fun saveLogFile(message: String) {
        val fileDir = File(rootPath.toString() + File.separator + PackageUtil.getAppProcessName(BaseApplication.application))
        if (!fileDir.exists()) {
            fileDir.mkdirs()
        }
