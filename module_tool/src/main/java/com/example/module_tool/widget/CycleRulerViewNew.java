...
public class CycleRulerViewNew extends View {
    ...
    public CycleRulerViewNew(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CycleRulerViewNew(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        int layerId = canvas.saveLayer(rectF,xpaint);
        width = getWidth();
        height = getHeight();
        radius = width / 2f- DeviceUtils.dp2px(getContext(),100f);
        paint.setColor(0x00000000);
        paint.setAntiAlias(true);
        paint.setStyle(Paint.Style.FILL);
        paint.setStrokeWidth(15f);
        offset = (height - radius) / 2f;
        oval.top=offset;
        oval.right=width;
