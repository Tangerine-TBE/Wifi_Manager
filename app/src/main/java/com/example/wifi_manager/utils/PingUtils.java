...
public class PingUtils
{
     ...
     * @param url 需要ping的url地址
     * @param count 需要ping的次数
     * @param timeout 需要ping的超时时间，单位 s
     * @return 平均RTT值，单位 ms 注意：-1是默认值，返回-1表示获取失败
     */
    ...
    public static void getAvgRTT(String url, int count, int timeout, PingCallBack callBack)
    {
        String domain = getDomain(url);
        if (null == domain)
        {
        ...
        if (null != pingString)
        {
            try
            {
                String tempInfo = pingString.substring(pingString.indexOf("min/avg/max/mdev") + 19);
                String[] temps = tempInfo.split("/");
                callBack.success(Math.round(Float.valueOf(temps[1])));
                return ;
            }
            catch (Exception e)
            {
